...
export default function AdminPage() {
  ...
  async function loadAll(t: string) {
    ...
    try {
      ...
      setUtilisateurs(((await usersRes.json()) as { utilisateurs: ProfileRow[] }).utilisateurs ?? []);
      setActivite(((await actRes.json()) as { serie: ActivitePoint[] }).serie ?? []);
      const evtData = await evtRes.json() as { erreurs: EvenementErreur[]; changements: ChangementStatut[] };
  ...
  if (!session) {
    return <LoginForm email={email} password={password} setEmail={setEmail} setPassword={setPassword} loginError={loginError} loginLoading={loginLoading} onSubmit={handleLogin} />;
  }
  ...
  return (
    <div className="flex min-h-screen" style={{ background: "var(--nuit-profonde)", color: "var(--ivoire)" }}>
      <Sidebar navItems={navItems} active={section} onSelect={(s) => { setSection(s); setMobileOpen(false); }} mobileOpen={mobileOpen} onToggle={() => setMobileOpen(!mobileOpen)} onLogout={handleLogout} />

      <main className="flex-1 overflow-y-auto px-5 py-6 md:px-8 md:py-10 lg:ml-64">
        <div className="mx-auto max-w-6xl">
          <div className="flex items-center justify-between mb-8 md:hidden">
            <button type="button" onClick={() => setMobileOpen(true)} className="btn-secondary text-xs px-4 py-2">
              ☰ Menu
            </button>
            <p className="caption uppercase text-xs" style={{ color: "var(--or-ancestral)" }}>SENYCE PARTNERS</p>
          </div>

          {loading && <p className="quote-italic">Chargement...</p>}
          {error && (
            <div className="rounded-lg border p-6 mb-6" style={{ borderColor: "rgba(224,122,107,0.45)" }}>
...
/*  Sidebar                                                           */
/* ------------------------------------------------------------------ */
function Sidebar({ navItems, active, onSelect, mobileOpen, onToggle, onLogout }: {
      ...
      <nav className="flex flex-col gap-1">
        {navItems.map((item) => (
          <button
            ...
            className="rounded px-4 py-3 text-left text-sm uppercase tracking-wider transition-colors"
            style={{
              background: active === item.id ? "rgba(201,168,76,0.12)" : "transparent",
              color: active === item.id ? "var(--or-ancestral)" : "rgba(254,252,240,0.72)",
              borderLeft: active === item.id ? "3px solid var(--or-ancestral)" : "3px solid transparent",
            }}
          >
            {item.label}
          </button>
  ...
  return (
    <>
      <aside className="fixed left-0 top-0 z-40 hidden h-full w-64 lg:block" style={{ borderRight: "1px solid rgba(201,168,76,0.15)" }}>
        {sidebar}
      </aside>
      ...
      {mobileOpen && (
        <div className="fixed inset-0 z-50 lg:hidden">
          ...
          <aside className="relative h-full w-72" style={{ borderRight: "1px solid rgba(201,168,76,0.15)" }}>
            {sidebar}
          </aside>
        </div>
      )}
...
/*  Login                                                             */
/* ------------------------------------------------------------------ */
function LoginForm({ email, password, setEmail, setPassword, loginError, loginLoading, onSubmit }: {
  email: string; password: string; setEmail: (v: string) => void; setPassword: (v: string) => void;
  loginError: string | null; loginLoading: boolean; onSubmit: (e: FormEvent) => void;
}) {
  return (
    <main className="min-h-screen flex items-center justify-center px-5" style={{ background: "var(--nuit-profonde)", color: "var(--ivoire)" }}>
      <form onSubmit={onSubmit} className="flex w-full max-w-sm flex-col gap-5 rounded-lg border p-8" style={{ borderColor: "rgba(201,168,76,0.3)" }}>
        <div className="text-center">
          <p className="caption uppercase mb-2" style={{ color: "var(--or-ancestral)" }}>SENYCE PARTNERS</p>
          <h1 className="h-display text-2xl" style={{ color: "var(--or-ancestral)" }}>Administration</h1>
          <p className="text-sm mt-2" style={{ color: "rgba(254,252,240,0.6)" }}>
            Identifie-toi pour accéder au tableau de bord.
          </p>
        </div>
        <label className="flex flex-col gap-2">
          <span className="caption uppercase text-xs" style={{ color: "rgba(237,217,154,0.78)" }}>Email</span>
          <input type="email" value={email} onChange={(e) => setEmail(e.target.value)}
            placeholder="admin@totemancestral.com" required className="form-input" autoComplete="email" />
        </label>
        <label className="flex flex-col gap-2">
          <span className="caption uppercase text-xs" style={{ color: "rgba(237,217,154,0.78)" }}>Mot de passe</span>
          <input type="password" value={password} onChange={(e) => setPassword(e.target.value)}
            placeholder="••••••••" required className="form-input" autoComplete="current-password" />
        </label>
        {loginError && <p className="text-sm text-center" style={{ color: "#E07A6B" }}>{loginError}</p>}
...
function OverviewSection({ stats }: { stats: AdminStats | null }) {
  ...
  return (
    <section>
      <h1 className="h-display text-3xl mb-2" style={{ color: "var(--or-ancestral)" }}>Vue d'ensemble</h1>
      ...
      <div className="grid gap-4 mb-10 md:grid-cols-3 lg:grid-cols-5">
        {cards.map((c) => (
          <article key={c.label} className="rounded-lg border p-5 text-center"
            style={{ background: "rgba(26,26,46,0.72)", borderColor: "rgba(201,168,76,0.22)" }}>
            <p className="caption uppercase text-xs mb-1" style={{ color: "rgba(237,217,154,0.72)" }}>{c.label}</p>
            <p className="h-display text-3xl" style={{ color: "var(--or-ancestral)" }}>{c.value}</p>
          </article>
        ))}
      </div>
    </section>
...
function OrdersSection({ commandes }: { commandes: CommandeRow[] }) {
  return (
    <section>
      <h2 className="h-display text-2xl mb-6" style={{ color: "var(--or-ancestral)" }}>Commandes</h2>
      ...
      {commandes.length === 0 ? (
        ...
        <div className="grid gap-3">
          {commandes.map((cmd) => (
            <div key={cmd.id} className="rounded-lg border p-4 grid gap-3 md:grid-cols-5 md:items-center"
              style={{ background: "rgba(26,26,46,0.72)", borderColor: "rgba(201,168,76,0.22)" }}>
              <div>
              ...
              <div>
                ...
                <p className="text-sm">{(cmd.montant_cents / 100).toFixed(2)} {cmd.devise?.toUpperCase()}</p>
              </div>
              <div>
...
function UsersSection({ utilisateurs }: { utilisateurs: ProfileRow[] }) {
  return (
    <section>
      <h2 className="h-display text-2xl mb-6" style={{ color: "var(--or-ancestral)" }}>Utilisateurs</h2>
      ...
      {utilisateurs.length === 0 ? (
        ...
        <div className="grid gap-3">
          {utilisateurs.map((u) => (
            <div key={u.id} className="rounded-lg border p-4 grid gap-3 md:grid-cols-5 md:items-center"
              style={{ background: "rgba(26,26,46,0.72)", borderColor: "rgba(201,168,76,0.22)" }}>
              <div>
              ...
              <div>
                ...
                <p className="text-sm">{u.total_commandes} ({(u.commandes_actives > 0) ? `${u.commandes_actives} active` : "0 active"})</p>
              </div>
              <div>
...
function ActivitySection({ activite }: { activite: ActivitePoint[] }) {
  ...
  return (
    <section>
      <h2 className="h-display text-2xl mb-2" style={{ color: "var(--or-ancestral)" }}>Activité (30 jours)</h2>
      <p className="quote-italic text-sm mb-6">Commandes, revenus et inscriptions quotidiennes.</p>

      <div className="rounded-lg border p-5 mb-8" style={{ background: "rgba(26,26,46,0.72)", borderColor: "rgba(201,168,76,0.22)" }}>
        <h3 className="text-sm uppercase tracking-wider mb-4" style={{ color: "var(--or-pale)" }}>Commandes par jour</h3>
        <ResponsiveContainer width="100%" height={280}>
          <BarChart data={activite}>
            <CartesianGrid strokeDasharray="3 3" stroke="rgba(201,168,76,0.08)" />
            <XAxis dataKey="date" tickFormatter={fmtDate} stroke="rgba(254,252,240,0.4)" tick={{ fontSize: 11 }} />
            <YAxis allowDecimals={false} stroke="rgba(254,252,240,0.4)" tick={{ fontSize: 11 }} />
            <Tooltip
              contentStyle={{ background: "#1A1A2E", border: "1px solid rgba(201,168,76,0.3)", borderRadius: 6, color: "#FEFCF0" }}
              labelFormatter={(v) => new Date(v).toLocaleDateString("fr-FR")}
            />
            <Legend />
            <Bar dataKey="commandes" name="Commandes" fill="#C9A84C" radius={[3, 3, 0, 0]} />
            <Bar dataKey="inscriptions" name="Inscriptions" fill="#EDD99A" radius={[3, 3, 0, 0]} />
          </BarChart>
        </ResponsiveContainer>
      </div>

      <div className="rounded-lg border p-5" style={{ background: "rgba(26,26,46,0.72)", borderColor: "rgba(201,168,76,0.22)" }}>
        <h3 className="text-sm uppercase tracking-wider mb-4" style={{ color: "var(--or-pale)" }}>Revenu par jour</h3>
        <ResponsiveContainer width="100%" height={280}>
          <LineChart data={activite}>
            <CartesianGrid strokeDasharray="3 3" stroke="rgba(201,168,76,0.08)" />
            <XAxis dataKey="date" tickFormatter={fmtDate} stroke="rgba(254,252,240,0.4)" tick={{ fontSize: 11 }} />
            <YAxis stroke="rgba(254,252,240,0.4)" tick={{ fontSize: 11 }} tickFormatter={(v: number) => `${(v / 100).toFixed(0)}€`} />
            <Tooltip
              contentStyle={{ background: "#1A1A2E", border: "1px solid rgba(201,168,76,0.3)", borderRadius: 6, color: "#FEFCF0" }}
              labelFormatter={(v) => new Date(v).toLocaleDateString("fr-FR")}
            ...
            />
            <Legend />
            <Line type="monotone" dataKey="revenu" name="Revenu" stroke="#C9A84C" strokeWidth={2} dot={{ fill: "#C9A84C", r: 3 }} />
          </LineChart>
        </ResponsiveContainer>
      </div>
    </section>
...
function EventsSection({ erreurs, changements, commandes, onRelancer, relanceLoading }: {
  ...
  return (
    <section>
      ...
      <div className="mb-8">
        <h3 className="text-sm uppercase tracking-wider mb-3" style={{ color: "var(--or-pale)" }}>Changements de statut</h3>
            ...
            {erreurs.map((e) => (
              <div key={e.id} className="rounded-lg border p-3"
                ...
                <div className="flex items-start justify-between mb-1">
                  <span className="font-mono text-xs" style={{ color: "#E07A6B" }}>{e.type ?? "Erreur"}</span>
...
function StatusBadge({ statut, className }: { statut: string; className?: string }) {
  const colors: Record<string, { border: string; color: string; bg: string }> = {
    livree: { border: "rgba(201,168,76,0.45)", color: "var(--or-ancestral)", bg: "rgba(45,45,26,0.58)" },
    paye: { border: "rgba(201,168,76,0.35)", color: "var(--or-pale)", bg: "rgba(26,26,46,0.86)" },
    en_generation: { border: "rgba(237,217,154,0.35)", color: "var(--or-pale)", bg: "rgba(26,26,46,0.86)" },
    en_attente_paiement: { border: "rgba(237,217,154,0.18)", color: "#888", bg: "rgba(26,26,46,0.86)" },
    erreur: { border: "rgba(224,122,107,0.45)", color: "#E07A6B", bg: "rgba(224,122,107,0.08)" },
    remboursee: { border: "rgba(224,122,107,0.25)", color: "#E07A6B", bg: "rgba(224,122,107,0.06)" },
  };
  const s = colors[statut] ?? { border: "rgba(237,217,154,0.28)", color: "var(--or-pale)", bg: "rgba(26,26,46,0.86)" };

  return (
    <span className={`inline-block rounded px-2 py-0.5 text-xs uppercase ${className ?? ""}`}

...
export async function GET(request: Request) {
  ...
  for (const j of jours) jourMap.set(j, { commandes: 0, revenu: 0, inscriptions: 0 });
  ...
  for (const c of commandes ?? []) {
    ...
    if (entry) {
      entry.commandes += 1;
      if (c.statut !== "remboursee" && c.statut !== "erreur") entry.revenu += c.montant_cents ?? 0;
    }
  }

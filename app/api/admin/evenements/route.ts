...
export async function GET(request: Request) {
  ...
  const { data: erreurs, error: errError } = await supabase
    ...
    .limit(50);
  if (errError) return NextResponse.json({ error: errError.message }, { status: 500 });
  ...
  const { data: changements, error: chgError } = await supabase
    ...
    .limit(20);
  if (chgError) return NextResponse.json({ error: chgError.message }, { status: 500 });

  return NextResponse.json({
    erreurs: (erreurs ?? []).map((e) => ({ ...e, type: e.etape })),

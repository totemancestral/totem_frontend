...
export async function GET(request: Request) {
  ...
  const [totalRes, revenueRes, statusRes, errorRes, todayRes, usersRes, oeuvresRes, oeuvresLivreesRes] =
    await Promise.all([
      supabase.from("commandes").select("*", { count: "exact", head: true }),
      supabase.from("commandes").select("montant_cents").in("statut", ["paye", "en_generation", "livree"]),
      supabase.from("commandes").select("statut", { count: "exact" }).in("statut", ["paye", "en_generation", "livree"]),

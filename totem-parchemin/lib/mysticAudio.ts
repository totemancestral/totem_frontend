// Ambiance sonore mystique 100% procédurale (Web Audio API).
// Aucune dépendance ni asset externe : un drone grave, un scintillement filtré,
// un souffle montant à l'ouverture et un carillon cristallin à la révélation.
...
class MysticAudio {
  ...
  // Démarre le drone d'ambiance. À appeler suite à un geste utilisateur.
  startAmbient() {
    ...
    // Drone : fondamentale + quinte légèrement désaccordées
    ...
    freqs.forEach((f, i) => {
      ...
      // LFO lent pour faire respirer le pad
    ...
    // Scintillement : bruit doux filtré passe-bande
  ...
  // Souffle ascendant lumineux (à l'ouverture du parchemin).
  swell() {
    const ctx = this.ensureCtx();
  ...
  // Carillon cristallin (arpège) à la révélation du contenu.
  chime() {
    const ctx = this.ensureCtx();
    if (!ctx || !this.master) return;
    // Gamme pentatonique majeure -> évoque le sacré
